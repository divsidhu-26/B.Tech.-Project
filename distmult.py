...
class TransE:
	...
	def test(self, inputs):
		...
		_, id_replace_head = tf.nn.top_k(tf.reduce_sum(tf.abs(calc(embedding_entity,rel_vec,tail_vec)), axis=1), k=self.__num_entity)
		_, id_replace_tail = tf.nn.top_k(tf.reduce_sum(tf.abs(calc(head_vec,rel_vec,embedding_entity)), axis=1), k=self.__num_entity)

		_, norm_id_replace_head = tf.nn.top_k(tf.reduce_sum(tf.abs(calc(norm_embedding_entity,norm_rel_vec,tail_vec)), axis=1), k=self.__num_entity)
		_, norm_id_replace_tail = tf.nn.top_k(tf.reduce_sum(tf.abs(calc(head_vec,norm_rel_vec,norm_embedding_entity)), axis=1), k=self.__num_entity)

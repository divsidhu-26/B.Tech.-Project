...
class Complex_multi(Model):
	def __init__(self, n_entities, n_relations, hparams):
		super(Complex_multi, self).__init__(n_entities, n_relations, hparams)
		self.l2_reg_lambda = hparams.l2_reg_lambda
		self.k = hparams.k
		self.build()
	...
	def add_params(self):
		self.entity_embedding1 = tf.Variable(tf.random_uniform([self.n_entities, self.embedding_size], 0., 1., seed=config.RANDOM_SEED), dtype=tf.float32, name="entity_embedding1")
		self.entity_embedding2 = tf.Variable(tf.random_uniform([self.n_entities, self.embedding_size], 0., 1., seed=config.RANDOM_SEED), dtype=tf.float32, name="entity_embedding2")
		self.relation_embedding1 = tf.Variable(tf.random_uniform([self.n_relations, self.k, self.embedding_size], 0., 1., seed=config.RANDOM_SEED), dtype=tf.float32, name="relation_embedding1")
		self.relation_embedding2 = tf.Variable(tf.random_uniform([self.n_relations, self.k, self.embedding_size], 0., 1., seed=config.RANDOM_SEED), dtype=tf.float32, name="relation_embedding2")
		self.U = tf.Variable(tf.random_uniform([self.n_relations, self.k], 0., 1., seed=config.RANDOM_SEED), dtype=tf.float32, name="U")
	...
	def add_prediction_op(self):
		...
		self.e2_1 = tf.nn.embedding_lookup(self.entity_embedding1, self.tails)
		self.e2_2 = tf.nn.embedding_lookup(self.entity_embedding2, self.tails)
		self.r1 = tf.nn.embedding_lookup(self.relation_embedding1, self.relations)
		self.r2 = tf.nn.embedding_lookup(self.relation_embedding2, self.relations)
		self.u = tf.nn.embedding_lookup(self.U, self.relations)

		e1_1 = tf.tile(tf.expand_dims(self.e1_1, 1), [1, self.k, 1])
		...
		self.pred = tf.reduce_sum(alpha * scores, -1, name="pred")
		
		#self.pred = tf.reduce_sum(self.u * tf.nn.tanh(scores), -1, name="pred")
...
class DEDICOM(Model):
	def __init__(self, n_entities, n_relations, hparams):
		super(DEDICOM, self).__init__(n_entities, n_relations, hparams)
		self.l2_reg_lambda = hparams.l2_reg_lambda
		self.build()
	
	def add_params(self):
		self.entity_embedding = tf.Variable(tf.random_uniform([self.n_entities, self.embedding_size], 0., 1., seed=config.RANDOM_SEED), dtype=tf.float32, name="entity_embedding")
		self.W = tf.Variable(tf.random_uniform([self.n_relations, self.embedding_size], 0., 1., seed=config.RANDOM_SEED), dtype=tf.float32, name="W")
	...
	def add_prediction_op(self):
		...
		v = tf.tile(tf.expand_dims(self.V, 0), [tf.shape(self.e1)[0], 1, 1])
		l = tf.expand_dims(self.e1 * self.w, 1)
		r = tf.expand_dims(self.e2 * self.w, 2)
		self.score = tf.squeeze(tf.matmul(tf.matmul(l, v), r)) 
		self.pred = tf.nn.sigmoid(self.score, name="pred")

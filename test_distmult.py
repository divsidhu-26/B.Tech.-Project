...
class TransE:
	...
	def train(self, inputs):
		...
		print(embedding_positive_relation.get_shape().as_list(),tf.shape(embedding_positive_head))

		score_positive = tf.reduce_sum(calc(embedding_positive_head,embedding_positive_relation,embedding_positive_tail),1,keepdims = False)
		score_negative = tf.reduce_sum(calc(embedding_negative_head,embedding_negative_relation,embedding_negative_tail),1,keepdims = False)

		print(score_positive.get_shape().as_list(),tf.shape(score_positive))

		res = -1*score_positive+score_negative
		loss_func = tf.reduce_mean(tf.nn.softplus(res))
		regul_func = tf.reduce_mean(embedding_positive_head**2) + tf.reduce_mean(embedding_positive_relation**2) + tf.reduce_mean(embedding_positive_tail**2) + tf.reduce_mean(embedding_negative_head**2) + tf.reduce_mean(embedding_negative_relation**2) + tf.reduce_mean(embedding_negative_tail**2)



		loss_every = loss_func
		loss_triple = tf.reduce_sum(loss_func)
		# self.__loss_regularizer = loss_regularizer = regul_func
	...
	def test(self, inputs):
		...
		_, id_replace_head = tf.nn.top_k(tf.reduce_sum(tf.abs(-1*calc(embedding_entity,rel_vec,tail_vec)), axis=1), k=self.__num_entity)
		_, id_replace_tail = tf.nn.top_k(tf.reduce_sum(tf.abs(-1*calc(head_vec,rel_vec,embedding_entity)), axis=1), k=self.__num_entity)

		_, norm_id_replace_head = tf.nn.top_k(tf.reduce_sum(tf.abs(-1*calc(norm_embedding_entity,norm_rel_vec,tail_vec)), axis=1), k=self.__num_entity)
		_, norm_id_replace_tail = tf.nn.top_k(tf.reduce_sum(tf.abs(-1*calc(head_vec,norm_rel_vec,norm_embedding_entity)), axis=1), k=self.__num_entity)
...
def main():
	...
	parser.add_argument('--data_dir', dest='data_dir', type=str, help='the directory of dataset', default='./data_0/')
	parser.add_argument('--learning_rate', dest='learning_rate', type=float, help='learning rate', default=0.01)
	parser.add_argument('--batch_size', dest='batch_size', type=int, help="batch size", default=5000)
	parser.add_argument('--max_iter', dest='max_iter', type=int, help='maximum iteration', default=1000)
	...
	parser.add_argument('--evaluate_per_iteration', dest='evaluate_per_iteration', type=int, help='evaluate the training result per x iteration', default=100)
	...
	with tf.Session() as session:
		saver.restore(session,'./data_0-distmult-100-1000')

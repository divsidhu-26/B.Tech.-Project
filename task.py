...
class Task:
    def __init__(self, model_name, data_name, cv_runs, params_dict, logger, eval_by_rel):
        ...
        if eval_by_rel:
            self.scorer = RelationScorer(self.train_triples, self.valid_triples, self.test_triples, self.n_relations)
        else:
    ...
    def cv(self):
        ...
        for i in range(self.cv_runs):
            ...
            print("sss",len(self.train_triples))
            ...
            self.logger.info("\t\t%d\t\t%d\t\t%f\t\t%f\t\t%f\t\t%f\t\t%f" % (i, step, res.raw_mrr, res.mrr,res.raw_hits_at1,res.raw_hits_at3,res.raw_hits_at10))
            m = self.scorer.compute_scores(pred_func,self.train_triples)
            self.logger.info("\t\t%d\t\t%d\t\t%f\t\t%f\t\t%f\t\t%f\t\t%f" % (i, step, m.raw_mrr, m.mrr,m.raw_hits_at1,m.raw_hits_at3,m.raw_hits_at10))
            cv_res.append(res)
            sess.close()
        ...
        print("CV Raw MRR: %.6f" % self.raw_mrr)
        print("CV Filtered MRR: %.6f" % self.mrr)
        print("Raw: Hits@1 %.3f Hits@3 %.3f Hits@10 %.3f" % (self.raw_hits_at1, self.raw_hits_at3, self.raw_hits_at10))
        print("Filtered: Hits@1 %.3f Hits@3 %.3f Hits@10 %.3f" % (self.hits_at1, self.hits_at3, self.hits_at10))
        self.logger.info("-" * 50)

    def refit(self, if_save=True):
        sess = self.create_session()
        sess.run(tf.global_variables_initializer())
        ...
        print("Test Results:")
        print("Raw MRR: %.6f" % res.raw_mrr)
        print("Filtered MRR: %.6f" % res.mrr)
        print("Raw: Hits@1 %.3f Hits@3 %.3f Hits@10 %.3f" % (res.raw_hits_at1, res.raw_hits_at3, res.raw_hits_at10))
        print("Filtered: Hits@1 %.3f Hits@3 %.3f Hits@10 %.3f" % (res.hits_at1, res.hits_at3, res.hits_at10))
...
class TaskOptimizer:
    ...
    def _obj(self, param_dict):
        ...
        self.task = Task(self.model_name, self.data_name, self.cv_runs, param_dict, self.logger, self.eval_by_rel)
        print("3")
        self.task.cv()
        # self.task.refit()
        print("3")
...
def main(options):
    ...
    optimizer = TaskOptimizer(options.model_name, options.data_name, options.max_evals, options.cv_runs, logger, options.relation)
    print("2")
    optimizer.run()

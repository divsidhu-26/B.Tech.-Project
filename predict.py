...
def parse_args(parser):
	parser.add_option("-d", "--data_name", default="bp", dest="data_name", type="string")
...
def main(options):
	if options.data_name == "data_25":
		e2id = load_dict_from_txt(config.DATA_25_E2ID)
		r2id = load_dict_from_txt(config.DATA_25_R2ID)
		n_entities = len(e2id)
	...
	checkpoint_file = os.path.join(config.CHECKPOINT_PATH, "DistMult")
	graph = tf.Graph()
